...
int write_stringz_as_srt(char *string, struct encoder_ctx *context, LLONG ms_start, LLONG ms_end)
{
    ...
    while (begin < unescaped + len)
    {
        ...
        // trim trailing spaces/tabs and dots
        {
            ...
            while (L && isspace((unsigned char)begin[L-1])) begin[--L] = '\0';
            while (L && begin[L-1] == '.')            begin[--L] = '\0';
        }

        size_t blen = strlen((const char *)begin);
        ...
        // advance past this line + the null terminator
        begin += blen + 1;
    }
